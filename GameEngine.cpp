...
GameEngine::GameEngine (FILE *fp) {
   ...
   string lastKey = string(""); // the key for the current parseMode (IE: the current room if PARSEMODEROOM)
      ...
      switch (tag) { // REMEMBER: at this point modstr only contains the parameter, tag contains the current tag
         case 'r': // room
            {
                ...
                * just make a complete checking function already
                ...
                map_room_descriptions.at(lastKey).compare(string("")) == 0) { // if desc is blank, none specified
               ...
               if (map_room_descriptions.find(lastKey) != map_room_descriptions.end()) {
                  fprintf(stderr, "line %d: Can't have duplicate room names\n", linecount);
                  exit(1);
               }
               if (map_room_descriptions.size() == 0) { // don't check tags if there are no rooms yet
                  map_room_descriptions.insert(pair<string, string>(lastKey, string(""))); // if unchanged, if
                  // statement above is trigerred
                  break;
               }
               auto opts_iter = map_room_opts.find(lastKey); // the next ~10 lines are for checking opts : tags
         ...
         case 'd': // description
            ...
            if (parseMode == PARSEMODENONE) {
               fprintf(stderr, "line %d: Cannot provide descriptions if not parsing for anything\n", linecount);
               exit(1);
            }
            if (parseMode == PARSEMODEROOM) {
               ...
               // this point
               map_room_descriptions.at(lastKey).append(string("\t") + string(modstr) + string("\n"));
         ...
         case 'o':
            {
               if (parseMode == PARSEMODENONE) {
                  fprintf(stderr, "line %d: Cannot provide options if not parsing for anything\n", linecount);
                  exit(1);
               }
               if (parseMode == PARSEMODEROOM) {
                  if (map_room_opts.find(lastKey) != map_room_opts.end() &&
                        ...
                        map_room_tags.find(lastKey)->second.size()) { // numopts should equal numtags at this point
         ...
         case 't':
            {
               if (parseMode == PARSEMODENONE) {
                  fprintf(stderr, "line %d: Cannot provide tags if not parsing for anything\n", linecount);
                  exit(1);
               }
               if (parseMode == PARSEMODEROOM) {
                  if (map_room_opts.find(lastKey) == map_room_opts.end() || // can't have tags before opts
                        (map_room_opts.find(lastKey) != map_room_opts.end() &&
                         map_room_tags.find(lastKey) != map_room_tags.end() &&
                         map_room_opts.find(lastKey)->second.size() ==
                         map_room_tags.find(lastKey)->second.size())) { // numopts should equal numtags - 1 at this point
